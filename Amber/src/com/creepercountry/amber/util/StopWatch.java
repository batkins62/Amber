...
public class StopWatch
{
	...
	public boolean newLoad(String task, long value)
	{
		...
		if (highestload < value)
		{
			...
			Notifier.log(NotifierLevel.LOWEST, "[WARNING] [StopWatch] [Peak] " + task + " -> cur." + loads.get(task).toString() + "/avg." + averageload);
			return true;
		}
		Notifier.log(NotifierLevel.DEBUG, "[INFO] [StopWatch] [Chirp] " + task + " -> cur." + loads.get(task).toString() + "/avg." + averageload);
		return true;
	}
