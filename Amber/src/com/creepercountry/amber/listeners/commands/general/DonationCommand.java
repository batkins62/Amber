...
public class DonationCommand extends BaseCommand
{
	...
	public boolean execute()
	{
		if (args.get(0).equalsIgnoreCase("expire"))
		{
			try
			{
				User user = sm.getUserHandler().getUser(args.get(1));		
				user.setValidDonator(false);
				plugin.getConsumer().queueUserSave(user);
				
				Notifier.sendMessage(NotifierType.REGULAR, sender, Colors.Green + String.format("Successfully expired player %s.", user.getName()));
			}
